...
import NetworkLogoHeader from '../components/NetworkHeader';
...
function NetworkAdminPage() {
  ...
  const handleLocationChange = (suggestion) => {
    ...
    if (suggestion) {
      // Update the event form with both the display name and coordinates if available
      setEventForm(prev => ({
        ...prev,
        location: suggestion.place_name,
        coordinates: suggestion.center ? {
          latitude: suggestion.center[1],  // Note: Mapbox returns [longitude, latitude]
  ...
  const handleOpenEventDialog = (mode, event = null) => {
    ...
    if (mode === 'edit' && event) {
      ...
      // Set location suggestion for the existing event
      if (event.location) {
                ...
                <CardContent>
                  ...
                  {event.coordinates && (
                    <Tooltip title={
                      ...
                      `Lat: ${event.coordinates.latitude?.toFixed(6) || '?'}, Lng: ${event.coordinates.longitude?.toFixed(6) || '?'}` :
        ...
        <DialogContent>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12} md={6}>
              ...
              {/* Replace standard TextField with AddressSuggestions */}
              <AddressSuggestions
