-- Migration: Fix comment reaction activity - get network_id from parent content
-- The comments table does not have a network_id column directly.
-- We need to join through the parent content (news, post, event, wiki) to get the network_id.
-- Created: 2025-12-04

-- Update create_reaction_activity function to properly handle comment reactions
...
BEGIN
    ...
    CASE NEW.content_type
        ...
        WHEN 'comment' THEN
            -- For comments, get network_id from the parent content
            -- Comments don't have network_id directly, need to join through parent
            SELECT
                COALESCE(nn.network_id, pi_prof.network_id, ne.network_id, wp.network_id),
                prof.full_name,
                'comment'
            INTO v_network_id, v_content_owner_name, v_content_title
            FROM comments c
            ...
            LEFT JOIN profiles pi_prof ON pi.profile_id = pi_prof.id
...
-- Add comment
COMMENT ON FUNCTION create_reaction_activity IS 'Creates activity feed entries when users react to content (supports post, news, comment, event, wiki, message; skips dm for privacy). Fixed to properly get network_id for comments by joining through parent content.';
