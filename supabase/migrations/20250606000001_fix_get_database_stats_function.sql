-- Fix the get_database_stats function SQL error
...
BEGIN
    ...
    -- Get individual table sizes (fix the GROUP BY issue)
    ...
    WITH index_stats AS (
        SELECT 
            ...
            SUM(pg_relation_size(psi.indexrelid)) as total_index_size,
            COUNT(*) FILTER (WHERE pi.indisunique) as unique_indexes,
            COUNT(*) FILTER (WHERE pi.indisprimary) as primary_indexes
...
-- Add comment
COMMENT ON FUNCTION get_database_stats() IS 'Returns comprehensive database statistics for super admin monitoring (fixed SQL)';