-- Add comment and comment_reply notification types to the notification_queue table constraint
-- This allows notifications for comments on posts and replies to comments
...
IS 'Allowed notification types: news, event, mention, direct_message, post, event_proposal (admin event reviews), event_status (event approval/rejection), event_reminder (24h before event), comment (comments on user posts), comment_reply (replies to user comments)';